...
export default function AIPhotoRestorer() {
  const t = useTranslations('Index.AIPhotoRestorer')
  ...
  const checkPredictionStatus = useCallback(async (retryCount = 0) => {
    ...
    try {
      const response = await fetch(`/api/ai/restore-photo?id=${predictionId}`)
      if (!response.ok) {
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                {isProcessing ? t('processingButton') : t('restoreButton')}
              </Button>
