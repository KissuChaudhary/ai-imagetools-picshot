...
import { Upload, X, AlertCircle, Download, ZoomIn } from 'lucide-react'

const MAX_FILE_SIZE = 5 * 1024 * 1024; // 5MB
const ALLOWED_FILE_TYPES = ['image/jpeg', 'image/png', 'image/webp'];

export default function AIImageUpscaler() {
  const t = useTranslations('Index.AIImageUpscaler')
  ...
  const fetchUsageCount = useCallback(async () => {
    try {
      const response = await fetch('/api/ai/upscale-image')
      if (response.ok) {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const response = await fetch('/api/ai/upscale-image', {
          ...
          body: JSON.stringify({ image: base64data, scale })
        })
  ...
  const checkPredictionStatus = useCallback(async (retryCount = 0) => {
    ...
    try {
      const response = await fetch(`/api/ai/upscale-image?id=${predictionId}`)
      if (!response.ok) {
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                {isProcessing ? t('processingButton') : t('upscaleButton')}
              </Button>
