...
export default function AIBackgroundRemover() {
  const t = useTranslations('Index.AIBackgroundRemover')
  ...
  const checkPredictionStatus = useCallback(async (retryCount = 0) => {
    ...
    try {
      const response = await fetch(`/api/ai/remove-background?id=${predictionId}`)
      if (!response.ok) {
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                {isProcessing ? t('processingButton') : t('removeBackgroundButton')}
              </Button>
