...
async function getUsageCount(ip: string) {
  ...
  const key = `background_remover_usage:${ip}:${date}`
...
export async function POST(req: Request) {
  try {
    ...
    if (usageCount >= MAX_USES_PER_DAY) {
      return NextResponse.json({ error: "You've reached the daily limit for background removal. Try again tomorrow!", usageCount }, { status: 429 })
    }
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (prediction.status === 'succeeded') {
      ...
      return NextResponse.json({ error: 'Background removal failed', usageCount }, { status: 500 })
    } else {
